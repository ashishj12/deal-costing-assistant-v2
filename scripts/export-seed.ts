...
async function main(): Promise<void> {
  ...
  for (const scenario of SCENARIOS) {
    ...
    for (const source of model.sources)
      write(`${dir}/sources/${source.name}`, source.content);
    ...
    if (!structure) {
      ...
      for (const q of store.state.model.questions)
        store.resolveQuestion(q.id, "Confirmed.");
      store.bulkSetStatus(
      ...
      await store.generateArchitecture(
        store.state.model.configuration.cloud ?? "recommend",
      );
  ...
  write(
    "expected-document-structure.md",
    [
  ...
  write(
    "estimation/commercial-config.json",
    json({
      defaults: SCENARIOS[0]!.build().configuration,
      perScenario: Object.fromEntries(
        SCENARIOS.map((s) => [s.id, s.build().configuration]),
      ),
      formula:
        "ROM = sum(workstream hours x role mix x hourly rate) x (1 + contingency); effort range = adjusted hours +/- band uncertainty",
    }),
  );
}
